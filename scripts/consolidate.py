...
def consolidate_predictions(
    ...
    for source_dir in source_dirs:
        for p in [p for p in (source_dir / postfix).iterdir() if p.is_file()]:
            shutil.copyfile(p, pred_dir / os.path.basename(p))
...
def main():
    ...
    shutil.copyfile(training_dirs[0] / "plan.pkl", target_dir / "plan.pkl")
    shutil.copyfile(training_dirs[0] / "config.yaml", target_dir / "config.yaml")
